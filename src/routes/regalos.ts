...
import { db } from "../db";
import { aplicarSubidaNivelViewer } from "./nivel19";
...
// Compra/envia un regalo: descuenta coins, suma puntos, registra el envio y notifica subida de nivel.
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const levelResult = await aplicarSubidaNivelViewer(client, {
        viewerId: viewer.id,
        usuarioId: viewer.usuario_id,
        streamId: stream.id,
        billeteraId: viewer.billetera_id,
        puntosTotales,
        nivelActual: viewer.nivel_actual,
      });
      const nivelParaMensaje = levelResult.nuevoNivel ?? viewer.nivel_actual;
      ...
      await client.query(
        ...
        [stream.id, viewer.usuario_id, msgText ?? null, regalo.id, envioRes.rows[0].id, nivelParaMensaje]
      );
      ...
      return res.status(201).json({
        ...
        saldo_restante: levelResult.saldoCoins ?? Number(billeteraRes.rows[0].saldo_coins),
        nivel_actual: nivelParaMensaje,
        subio_nivel: levelResult.subioNivel,
        recompensa_coins: levelResult.subioNivel ? levelResult.recompensaCoins ?? 0 : 0,
        notificacion_id: levelResult.notificacionId ?? null,
